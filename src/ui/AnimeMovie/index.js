...
class AnimeStream extends React.Component{
    ...
    async sourceGet(){
        this.onloading()
        let sources = []
        let getstreamdata = await fetch(API.getstreamdata+this.props.epsid).then((res) => res.json())
        let title = getstreamdata.data.title

        let getstream = getstreamdata.data.stream
        let mega = {
            data: getstreamdata.data.mega
        }
